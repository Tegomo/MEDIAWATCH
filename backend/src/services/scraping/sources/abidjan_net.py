"""Scraper pour Abidjan.net - Portail d'information ivoirien"""
import httpx
...
class AbidjanNet(MediaScraper):
    """Scraper pour le site Abidjan.net"""
    ...
    async def get_article_urls(self, max_pages: int = 3) -> list[str]:
        ...
        async with httpx.AsyncClient(
            ...
            for page in range(1, max_pages + 1):
                try:
                    if page == 1:
                        response = await client.get(f"{self.base_url}/articles")
                    else:
                        response = await client.get(f"{self.base_url}/articles?page={page}")
                    ...
                    for node in tree.css("a.article-link, .article-item a, h2 a[href], h3 a[href]"):
                        href = node.attributes.get("href", "")
                        if href and href.startswith(("http", "/")):
                            if href.startswith("/"):
                                href = f"{self.base_url}{href}"
                            if "abidjan.net" in href and href not in urls:
                                urls.append(href)
    ...
    async def parse_article(self, url: str) -> Optional[ScrapedArticle]:
        ...
        # Titre
        title_node = tree.css_first("h1.article-title, h1.entry-title, h1")
        if not title_node:
            return None
        ...
        # Auteur
        author = None
        author_node = tree.css_first(".article-author, .author, [rel='author']")
        if author_node:
            author = author_node.text(strip=True)

        # Date
        published_at = datetime.utcnow()
        date_node = tree.css_first("time[datetime], .article-date, .date, .published")
        if date_node:
